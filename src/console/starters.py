...
from console import domains
from console.db.models import Project

CONSOLE_TOML_TEMPLATE = """\
# console.toml for {name}
# Lives at the repo root. The console reads this file from the repo at every
# build, and it is validated before anything touches Docker. You can check
# edits anytime on the console's "toml check" page.
...
DOCKERFILE_TEMPLATE = """\
# Dockerfile for {name}
# The console builds this on the server on every push to {branch}, pushes
# the image to GHCR, and deploys it. The build targets the server's
